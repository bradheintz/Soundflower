    ...
    switch(inPropertyID)
    {
            ...
            if (isInput) {
                ...
                if (gThruEngine2->IsRunning() && gThruEngine2->GetInputDevice() == inDevice){
                    ...
                    [app srChanged2ch];
                }else if (gThruEngine16 && gThruEngine16->IsRunning() && gThruEngine16->GetInputDevice() == inDevice){
                ...
                if (inChannel == 0) {
                    ...
                    }else if (gThruEngine16 && gThruEngine16->IsRunning() && gThruEngine16->GetOutputDevice() == inDevice){
            ...
            if (!isInput) {
                if (inChannel == 0) {
                    if (gThruEngine2->GetOutputDevice() == inDevice || (gThruEngine16 && gThruEngine16->GetOutputDevice() == inDevice)) {
...
- (IBAction)suspend
{
    ...
    mSuspended16chDeviceID = (gThruEngine16)? gThruEngine16->GetOutputDevice():kAudioDeviceUnknown;
...
- (IBAction)checkNchnls
{
    ...
    if (gThruEngine16 && (mNchnls16 != gThruEngine16->GetOutputNchnls()))
    {
        NSMenuItem	*curdev = mCur16chDevice;
        [self outputDeviceSelected:[mMenu itemAtIndex:m16StartIndex]];
        //usleep(1000);
...
{   // Sometimes, connecting a new device (e.g. HDMI) causes an annoying buzzing noise that is only stopped when the buffer is cleared. We achieve this by resetting the buffer size after a small delay since the device (e.g. HDMI) was connected
    gThruEngine2->SetBufferSize(64 << [m2chBuffer indexOfItem:mCur2chBufferSize]);
    NSLog(@"Resetting buffer size!");
}
...
- (IBAction)refreshDevices
{
    ...
    NSMenuItem *mSelectedDevice = [mMenu itemAtIndex:m2StartIndex+1];   // Select the highest priority device

    /*for (int i = 0 ; i < 64 ; i++){
        if (mMenuID2[i] == dev){
            mSelectedDevice = [mMenu itemAtIndex:m2StartIndex+1+i];   // Unless the previously selected device is still present
            break;
        }
    }*/
...
- (void)buildDeviceList
{
    ...
    AudioDeviceList::DeviceList &thelist = mOutputDeviceList->SortList();
    for (AudioDeviceList::DeviceList::iterator i = thelist.begin(); i != thelist.end(); ++i) {
        if (0 == strncasecmp("Soundflower", (*i).mName, strlen("Soundflower"))) {
            if (0 == strcasecmp("Soundflower (2ch)", (*i).mName)) {
...
- (void)awakeFromNib
{
    ...
    [mSbItem setImage:[NSImage imageNamed:@"sunflower-icon_small"]];    // Added these two images as the notification bar icon, looks cooler :)
    [mSbItem setAlternateImage:[NSImage imageNamed:@"sunflower_small"]];
    ...
    if (mSoundflower2Device) {
        ...
        [self refreshDevices];  // Choose device based on priority (Comment this line to select the device that the user had selected right before quitting Soundflower)
    }
...
- (IBAction)outputDeviceSelected:(id)sender
{
    ...
    if (val < m16StartIndex) {
        ...
        AudioDeviceID sysOutDevId = (val<0)? mDefaultDevice:mSoundflower2Device;// ID of the device to be selected as the system's output (avoids the user having to open System Preferences to choose Soundflower)
        AudioDevice outDevice((val<0? mDefaultDevice:mMenuID2[val]), false);
        if (gThruEngine2->GetOutputDevice() != outDevice.mID) { // Only set output device if the new id is different. That way, sound is not instantly discontinued when the same id is selected :)
            gThruEngine2->SetOutputDevice(outDevice.mID);   // Even when they select None, we can set the output device to the default output, so that in case the user selects Soundflower as the output device, they still get the default audio output
        }
...
- (void)readGlobalPrefs
{
    ...
    if (num) {
        ...
        if (num) CFRelease(num);    // For some reason, CFNumberGetValue causes num to become NULL and therefore CFRelease(NULL) would crash
...
{
    CFStringRef cfstr; UInt32 val; CFNumberRef number;
    if (mSoundflower2Device) {
        cfstr = CFStringCreateWithCString(kCFAllocatorSystemDefault, [[mCur2chDevice title] cString], kCFStringEncodingMacRoman);
        CFPreferencesSetAppValue(CFSTR("2ch Output Device"), cfstr, kCFPreferencesCurrentApplication);
        CFRelease(cfstr);

        val = 64 << [m2chBuffer indexOfItem:mCur2chBufferSize];
